...
class NotificationService: UNNotificationServiceExtension {
    ...
    //MARK: - 下载方法
    func downloadAndSave(url:URL,handler:@escaping (_ loadURL:URL?)->Void) -> Void{
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, res, error) in
            ...
            if let data = data {
                ...
                let catchURL = URL(fileURLWithPath: FileManager.default.cachesDirectory)
                
                let url = catchURL.appendingPathComponent(url.absoluteString.md5).appendingPathExtension(ext)
